		...
		while(loop) 
		{
			...
			if(input.equals("acc"))
			{
					...
					//ask for the initial deposit
				{
					...
					/**
					 * MAKE SURE THAT THE INPUT IS A DOUBLE AND DONT LET IT CRASH FOR THE DEPOSIT
					 * 
					 * try// catch
					 */
					
					if(type.equals("check"))
						accounts.add(new CheckingAccount (name, deposit, OVER_DRAFT_FEE, TRANSACTION_FEE, FREE_TRANSACTIONS ));
					
					if(type.equals("save"))
						accounts.add(new SavingsAccount(name, deposit, RATE, MIN_BAL, MIN_BAL_FEE ));
